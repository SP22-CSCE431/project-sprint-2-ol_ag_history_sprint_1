...
RSpec.describe(Lineage, type: :model) do
  ...
  it 'Same Member id' do
    testlineage = described_class.create!(member_id: testMember2.id, big: testMember1.id, little: testMember3.id)
    subject.member_id = testMember2.id
    expect(subject).not_to(be_valid)
  end

  it 'Same Big' do
    testlineage = described_class.create!(member_id: testMember2.id, big: testMember3.id, little: testMember1.id)
